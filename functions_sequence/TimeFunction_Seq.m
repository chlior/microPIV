function [handles]=TimeFunction(hObject, eventdata, handles)
    ...
    handles.fps = str2num(get(handles.edit10,'string'))
...
profile = Vnan(:,sec);
umax = max(profile);
uavg(i) = nanmean(profile);  
Q(i) = mean(uavg(i))*handles.channelWidth*handles.channelHeight % Q [um^3/sec]
QL(i) = Q(i)*60*10e-9 % Q [ul/min]
